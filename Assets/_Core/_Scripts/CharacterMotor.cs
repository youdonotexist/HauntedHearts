...
public class CharacterMotor : MonoBehaviour
{
    ...
    public class CharacterMotorMovement
    {
        ...
        public float maxSidewaysSpeed = 7.5f;
    ...
    private Vector3 ApplyInputVelocityChange(Vector3 velocity)
    {
        if(!canControl)
			...
			inputMoveDirection = new Vector3(Input.GetAxis("Horizontal"), Input.GetAxis("Vertical"), 0.0f);
    ...
    {
		inputJump = Input.GetAxis("Jump") > 0.0f;
        ...
        if(grounded) {
			//transform.up = Vector3.Slerp(transform.up, lastGroundNormal, Time.deltaTime * 20.0f);
        ...
        else
        {
			...
			transform.up = Vector3.Slerp(transform.up, Vector3.up, Time.deltaTime * 5.0f);
        }
